...
@Service
public class ListPostsByCategoryService {

    @Autowired
    PostRepository postRepository;

    public Page<PostResponse> listPostsByCategory(DonationType type, Pageable pageable) {

        Page<Post> postsByType = postRepository.findByCategory(type, pageable);

        List<PostResponse> response = postsByType.stream().map(PostMapper::toResponse).collect(Collectors.toList());
